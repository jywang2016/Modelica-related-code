...
model HVACv4a_WL_Fault3
  ...
  Buildings.Fluid.FixedResistances.Junction junLeak(redeclare package Medium = air, dp_nominal = {30, -30, 30}, m_flow_nominal = {1, -0.5, -0.5})  annotation(
    Placement(visible = true, transformation(origin = {90, 40}, extent = {{-4, -4}, {4, 4}}, rotation = -90)));
equation
  connect(junLeak.port_2, senSAT.port_a) annotation(
    Line(points = {{90, 36}, {90, 36}, {90, 14}, {90, 14}}, color = {0, 127, 255}));
  connect(junLeak.port_3, Outside.ports[3]) annotation(
    Line(points = {{86, 40}, {-78, 40}, {-78, 58}, {-78, 58}}, color = {0, 127, 255}));
  connect(junCooling.port_2, junLeak.port_1) annotation(
    Line(points = {{86, 72}, {90, 72}, {90, 44}, {90, 44}}, color = {0, 127, 255}));
