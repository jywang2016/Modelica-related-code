...
model HVACv3a_WL_Fault3
  // v3a_WL_Fault3:
  // Add in fault3: leaking air duct simulated by adding a T-joint valve that connects to outside
  // v3a_weather_load:
  ...
  Buildings.Fluid.Sources.Boundary_pT Outside(redeclare package Medium = air, nPorts = 3, use_T_in = true) annotation(
  ...
  TestSys.ControlUnitType2 controlUnit(kd = 0, ki = 2.51e-4, kp = 0.09, maxPower = 1, minPower = 0.005, setSAT = false, uFan = 500) annotation(
  ...
  Buildings.Fluid.Sensors.TemperatureTwoPort senSAT(redeclare package Medium = air, TAmb = system.T_ambient, T_start = system.T_ambient, m_flow_nominal = 1) annotation(
    Placement(visible = true, transformation(origin = {90, 8}, extent = {{-6, 6}, {6, -6}}, rotation = -90)));
  ...
  Buildings.Fluid.FixedResistances.Junction junLeak(redeclare package Medium = air, dp_nominal = {30, -30, 30}, m_flow_nominal = {1, -0.5, -0.5}) annotation(
    Placement(visible = true, transformation(origin = {90, 54}, extent = {{-4, 4}, {4, -4}}, rotation = -90)));
equation
  connect(junLeak.port_3, Outside.ports[3]) annotation(
    Line(points = {{94, 54}, {96, 54}, {96, 42}, {-76, 42}, {-76, 58}, {-78, 58}}, color = {0, 127, 255}));
  connect(junLeak.port_2, senSAT.port_a) annotation(
    Line(points = {{90, 50}, {90, 14}}, color = {0, 127, 255}));
  connect(senSAT.T, controlUnit.uSAT) annotation(
    Line(points = {{83, 8}, {83, -2}, {-62, -2}, {-62, 14}, {-58, 14}}, color = {0, 0, 127}));
  connect(senSAT.port_b, Room.ports[1]) annotation(
    Line(points = {{90, 2}, {90, -6}, {74, -6}, {74, -24}}, color = {0, 127, 255}));
  connect(junCooling.port_2, junLeak.port_1) annotation(
    Line(points = {{86, 72}, {90, 72}, {90, 58}, {90, 58}}, color = {0, 127, 255}));
  connect(controlUnit.yHeating, swValHeating.u) annotation(
    Line(points = {{-44, 10}, {-6, 10}, {-6, 44}, {12, 44}, {12, 56}, {12, 56}}, color = {0, 0, 127}));
  connect(controlUnit.ySAD, ahu1.y_SA) annotation(
    Line(points = {{-44, 22}, {-12, 22}, {-12, 74}, {-24, 74}, {-24, 66}, {-24, 66}, {-24, 66}}, color = {0, 0, 127}));
