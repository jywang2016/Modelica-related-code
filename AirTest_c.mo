...
model AirTest_c
  ...
  Buildings.Airflow.Multizone.Orifice orifice(redeclare package Medium = Air, A = 0.001) annotation(
    Placement(visible = true, transformation(origin = {84, 64}, extent = {{-4, -4}, {4, 4}}, rotation = 0)));
  Buildings.Fluid.MixingVolumes.MixingVolume volOutside(redeclare package Medium = Air, V = 1e10, nPorts = 3) annotation(
    Placement(visible = true, transformation(origin = {-85, 33}, extent = {{-9, -9}, {9, 9}}, rotation = 90)));
...
equation
  connect(orifice.port_b, volOutside.ports[3]) annotation(
    Line(points = {{88, 64}, {98, 64}, {98, -44}, {-76, -44}, {-76, 34}}, color = {0, 127, 255}));
  connect(volRoom.ports[2], orifice.port_a) annotation(
    Line(points = {{68, 64}, {74, 64}, {74, 63.75}, {80, 63.75}, {80, 64}}, color = {0, 127, 255}));
  connect(damper1.port_b, volRoom.ports[1]) annotation(
    Line(points = {{56, 84}, {68, 84}, {68, 64}}, color = {0, 127, 255}));
  connect(volRoom.ports[3], fan2.port_a) annotation(
    Line(points = {{68, 64}, {68, 25.5}, {54, 25.5}, {54, -12}}, color = {0, 127, 255}));
  connect(volOutside.ports[2], mixingBox.port_Exh) annotation(
    Line(points = {{-76, 34}, {-72, 34}, {-72, 28}, {-56, 28}, {-56, 28}}, color = {0, 127, 255}));
  connect(volOutside.ports[1], mixingBox.port_Out) annotation(
    Line(points = {{-76, 34}, {-72, 34}, {-72, 40}, {-56, 40}, {-56, 40}}, color = {0, 127, 255}));
