...
model HVACSystemTest
  package air = Buildings.Media.Air "Medium air";
  package water = Buildings.Media.Water "Medium water";  
  
  Buildings.Fluid.MixingVolumes.MixingVolume Room(redeclare package Medium = air, T_start = 273.15 + 30 - 25, V = 10 * 10 * 3, m_flow_nominal = 1, nPorts = 2) annotation(
  ...
  Buildings.Fluid.Sensors.TemperatureTwoPort senToutside(redeclare package Medium = air, m_flow_nominal = 1) annotation(
  ...
  Modelica.Blocks.Sources.Constant OutsideTemp(k = 303.15) annotation(
