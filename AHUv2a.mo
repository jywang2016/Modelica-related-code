...
model AHUv2a
  ...
  Buildings.Fluid.Movers.FlowControlled_dp fanSA(redeclare package Medium = Medium1,dp(start = 2000), dp_nominal = 1000, m_flow(start = 0.1), m_flow_nominal = 1)  annotation(
    Placement(visible = true, transformation(origin = {81, 21}, extent = {{-7, -7}, {7, 7}}, rotation = -90)));
  Modelica.Blocks.Interfaces.RealInput y_fanSA annotation(
    Placement(visible = true, transformation(origin = {99, 31}, extent = {{9, -9}, {-9, 9}}, rotation = 0), iconTransformation(origin = {76, 80}, extent = {{8, -8}, {-8, 8}}, rotation = 0)));
equation
  connect(y_fanSA, fanSA.dp_in) annotation(
    Line(points = {{99, 31}, {98, 31}, {98, 20}, {90, 20}, {90, 22}}, color = {0, 0, 127}));
  connect(fanSA.port_b, port_SA) annotation(
    Line(points = {{82, 14}, {80, 14}, {80, -12}, {82, -12}}, color = {0, 127, 255}));
  connect(jun_CA.port_2, fanSA.port_a) annotation(
    Line(points = {{86, 92}, {96, 92}, {96, 38}, {80, 38}, {80, 28}, {82, 28}}, color = {0, 127, 255}));
