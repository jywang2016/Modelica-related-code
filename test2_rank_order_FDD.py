...
# plt.figure()
# plt.plot(heatload_avg,'b')
# plt.plot(heatload_avg + heatload_std,'b--')
# plt.plot(heatload_avg - heatload_std,'b--')
# plt.fill_between(heatload_df.index,heatload_Q1,heatload_Q3,color = 'blue', alpha = 0.2)
# plt.xlabel('Time[hours]')
# plt.ylabel('Temperature[K]')
# plt.title('Day Temperature')
# plt.show()
...
# Load data
# testdata_name = '102_HVACv4a_Weather+HeatLoadDataTest_Boston+SmallOffice'
# testdata_name = '103_HVACv4a_Weather+HeatLoadDataTest_Boston+SmallOffice_Fault1'
# testdata_name = '104_HVACv4a_Weather+HeatLoadDataTest_Boston+SmallOffice_Fault2'
# testdata_name = '105_HVACv4a_Weather+HeatLoadDataTest_Boston+SmallOffice_Fault3'
# testdata_name = '111_HVACv4a_Weather+HeatLoadDataTest_Boston+SmallOffice_Fault5'
# testdata_name = '112_HVACv4a_Weather+HeatLoadDataTest_Boston+SmallOffice_Fault6'
testdata_name = '110_HVACv4a_Weather+HeatLoadDataTest_Boston+SmallOffice_random'
...
#
experimentName = testdata_name + '_LOF' #'test1'
...
for MV_index in range(len(MVs)):
    ...
    for n in range(test_weather_ts.shape[0]):
        # n = 0 # sample index
        # weather_group = pattern_groups[str(test_weather_sax[n])]
        
        # The 20th closest weather data
        weather_group = rank_group(weather_ts,test_weather_ts[n])['Day'][:20]
        # heatload_group = pattern_groups_h[str(test_heatload_sax[n])]
        
        # combination = list(pattern_group_intersection(weather_group,heatload_group))
        # combination.sort()
        
        # print('{} - combination length:{}'.format(n,len(combination)))
        ...
        if pred[-1] == -1:
            ...
            dir_loc = r'N:\HVAC_ModelicaModel_Data\python_figs\rank\{}\{}'.format(experimentName,MVs[MV_index])
...
# print results:
fd_rate = 'Fault detection rate:\t {}%'.format(len(fault_index)/test_EV_ts.shape[0]*100)
nd_rate = 'Normal operation rate:\t {}%'.format(len(normal_index)/test_EV_ts.shape[0]*100)
ld_rate = 'Lack of data rate:\t {}%'.format(len(lack_index)/test_EV_ts.shape[0]*100)
...
# Save results:
dir_loc = r'N:\HVAC_ModelicaModel_Data\python_figs\rank\{}'.format(experimentName)
...
#
experimentName = testdata_name + '_IsolationForest' #'test1'
# Choose ranking method
rank_group = rank_high_low
