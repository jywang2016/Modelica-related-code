...
file = r'L:\HVAC_ModelicaModel_Data\python_figs\rank_Boston+SmallOffice_Workday\all_contamination_classified.csv'
data_row = 4
data_col = 2
...
TPR = df.loc[algorithms[1]][experiments[1]][dist_measures[0]][rates[1]]
FPR = df.loc[algorithms[1]][experiments[4]][dist_measures[0]][rates[1]]
...
for e,experiment in enumerate(exp_list):
    ...
    for d,d_measure in enumerate(dist_measures):
        for a,aa in enumerate(algorithms):
            TPR = df.loc[algorithms[a]][experiments[1]][dist_measures[d]][rates[1]]
