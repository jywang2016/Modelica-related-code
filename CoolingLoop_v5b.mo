...
model CoolingLoop_v5b
  ...
  // v5a: add in parameter to set cool water temperature
  // v5b: rearrange tank2 and cooling tower to avoid overheated water in tank2. Increase tank2 cross area
  
  parameter Real chilled_water_T = 273.15+5 "chilled water temperature";
  parameter Real condensePumpGain = 3 "gain for the condense water pump";
  ...
  Modelica.Fluid.Vessels.OpenTank tank2(redeclare package Medium = water, crossArea = 4, height = 1.5, nPorts = 2, use_portsData = false) annotation(
  ...
  Modelica.Blocks.Sources.Constant TEva_const(k = chilled_water_T) annotation(
    Placement(visible = true, transformation(origin = {-14, 42}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  Modelica.Fluid.Interfaces.FluidPort_a port_a(redeclare package Medium = water) "Hot water in" annotation(
  ...
  Modelica.Fluid.Interfaces.FluidPort_b port_b(redeclare package Medium = water) "Chilled water out" annotation(
  ...
  Modelica.Blocks.Math.Gain coolingPumpGain(k = condensePumpGain)  annotation(
    Placement(visible = true, transformation(origin = {68, -82}, extent = {{-6, -6}, {6, 6}}, rotation = 0)));
equation
  connect(coolingPumpGain.y, pumpCoolingTower.m_flow_in) annotation(
    Line(points = {{74, -82}, {96, -82}, {96, 44}, {52, 44}, {52, 52}, {52, 52}}, color = {0, 0, 127}));
  connect(yCooling, coolingPumpGain.u) annotation(
    Line(points = {{2, -82}, {61, -82}}, color = {0, 0, 127}));
  connect(tank2.ports[2], pumpCoolingTower.port_a) annotation(
    Line(points = {{80, -4}, {76, -4}, {76, -10}, {62, -10}, {62, 64}, {62, 64}}, color = {0, 127, 255}));
  connect(CoolingTower.port_b, tank2.ports[1]) annotation(
    Line(points = {{64, -52}, {82, -52}, {82, -4}, {80, -4}}, color = {0, 127, 255}));
  connect(chiller.port_b1, CoolingTower.port_a) annotation(
    Line(points = {{8, 2}, {8, 2}, {8, -52}, {44, -52}, {44, -52}}, color = {0, 127, 255}));
  connect(prescribedTemperature.port, tank2.heatPort) annotation(
    Line(points = {{-44, 32}, {-26, 32}, {-26, -16}, {68, -16}, {68, 7}}, color = {191, 0, 0}));
