...
model HVACv4_tune
  ...
  TestSys.ControlUnitType2 controlUnit( kd = 0,ki = 3.49e-5, kp = 0.09, maxPower = 1, minPower = 0.005, setSAT = false, uFan = 2000)  annotation(
  ...
  Modelica.Blocks.Sources.Constant TemperatureSetpoint(k = 273.15 + 22)  annotation(
    Placement(visible = true, transformation(origin = {-88, 8}, extent = {{-8, -8}, {8, 8}}, rotation = 0)));
  Modelica.Blocks.Sources.Step step1(height = 3, offset = 288.15, startTime = 7200)  annotation(
    Placement(visible = true, transformation(origin = {-87, 33}, extent = {{-5, -5}, {5, 5}}, rotation = 0)));
equation
  connect(step1.y, controlUnit.uSP) annotation(
    Line(points = {{-82, 34}, {-72, 34}, {-72, 8}, {-58, 8}, {-58, 8}}, color = {0, 0, 127}));
  connect(OutsideTemp.y, Outside.T_in) annotation(
    Line(points = {{-86, 92}, {-74, 92}, {-74, 80}, {-74, 80}}, color = {0, 0, 127}));
