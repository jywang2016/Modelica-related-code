...
model HVACv3a_WL_Fault6
  // v3a_WL_Fault6:
  // Add in fault6: malfunctioning thermostat. This is done by adding a pulse to the RAT sensor
  // v3a_weather_load:
  ...
  Modelica.Blocks.Sources.Pulse pulse1(amplitude = 2, offset = -2, period = 1200)  annotation(
    Placement(visible = true, transformation(origin = {-17, -29}, extent = {{-5, -5}, {5, 5}}, rotation = 90)));
  Modelica.Blocks.Math.Add add1 annotation(
    Placement(visible = true, transformation(origin = {-35, -11}, extent = {{5, -5}, {-5, 5}}, rotation = 0)));
equation
  connect(add1.y, controlUnit.uRAT) annotation(
    Line(points = {{-40, -10}, {-66, -10}, {-66, 18}, {-58, 18}, {-58, 18}}, color = {0, 0, 127}));
  connect(pulse1.y, add1.u2) annotation(
    Line(points = {{-17, -23}, {-17, -14}, {-28, -14}}, color = {0, 0, 127}));
  connect(senRAT.T, add1.u1) annotation(
    Line(points = {{20, -18}, {20, -8}, {-29, -8}}, color = {0, 0, 127}));
